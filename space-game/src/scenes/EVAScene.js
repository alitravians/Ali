  ...
  update(delta) {
    ...
    if (nearTarget && !nearTarget.done) {
      ...
      this.gs.ui.addElement('eva-hint', `
        <div style="position:fixed;bottom:130px;left:50%;transform:translateX(-50%);
          ...
          <div style="color:rgba(100,160,255,0.7);font-size:0.72rem;margin-top:3px;">استمر بالضغط على F — ${stepName}</div>
          <div style="color:rgba(100,160,255,0.3);font-size:0.62rem;">خطوة ${nearTarget.currentStep + 1} من ${nearTarget.steps.length}</div>
        </div>
      `);
    }
    ...
    this.gs.ui.addElement('eva-tasks', `
      <div style="position:fixed;top:70px;right:15px;background:rgba(0,0,0,0.85);
        border:1px solid rgba(100,160,255,0.15);border-radius:2px;padding:10px 14px;direction:rtl;max-width:220px;">
    ...
    this.gs.ui.addElement('suit-systems', `
      <div style="position:fixed;top:70px;left:15px;background:rgba(0,0,0,0.85);
        border:1px solid rgba(74,222,128,0.1);border-radius:2px;padding:10px 14px;direction:rtl;">
