#!/usr/bin/env python3
# ──────────────────────────────────────────────────────────────────────────
# Injector: Day/Night cycle + server clock HUD (تعاقب الليل والنهار + ساعة السيرفر).
#   1) Adds/updates Script  "DayNightCycle" in ServerScriptService
#      (source = src/DayNightCycle.server.lua).
#   2) Adds/updates LocalScript "DayNightClient" in
#      StarterPlayer.StarterPlayerScripts (source = src/DayNightClient.client.lua).
#
...
SCRIPTS = [
    ("Script",      "DayNightCycle", "DayNightCycleServerRef",
     os.path.join(HERE, "src", "DayNightCycle.server.lua"), "ServerScriptService"),
    ("LocalScript", "DayNightClient", "DayNightClientRef",
     os.path.join(HERE, "src", "DayNightClient.client.lua"), "StarterPlayerScripts"),
]
