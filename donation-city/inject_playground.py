#!/usr/bin/env python3
# Injects PlaygroundArea (Script, after ParkourSystem) into the rbxlx. Idempotent.
import sys
...
if 'referent="PlaygroundAreaRef"' not in d:
    src = read("src/PlaygroundArea.server.lua")
    at = item_end_after(d, "ParkourSystemRef")
    d = d[:at] + make_item("Script", "PlaygroundAreaRef", "PlaygroundArea", src, 4) + d[at:]
    print("Injected PlaygroundArea (%d chars)" % len(src))
else:
    print("PlaygroundArea already present")
