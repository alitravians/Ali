...
-- 🎟️ لوحة اسم بطابع «تذكرة سينما» — نسخة مطابقة تماماً للموجودة في CinemaSystem
-- (هالة + بطاقة داكنة + إطار خفيف + شريط اسم علوي + وصف + خط أكسنت رفيع بديل النقاط)
...
local function makeCinemaNameTag(head, opts)
	...
	-- هالة خفيفة خلف البطاقة (ظل بلون الدور)
	local glow = Instance.new("Frame")
	glow.Name = "Glow"; glow.Size = UDim2.new(1, 8, 1, 8)
	glow.Position = UDim2.new(0, -4, 0, -4)
	glow.BackgroundColor3 = accent; glow.BackgroundTransparency = 0.82
	glow.BorderSizePixel = 0; glow.ZIndex = 0; glow.Parent = bb
	Instance.new("UICorner", glow).CornerRadius = UDim.new(0, 14)

	-- البطاقة الرئيسية (داكنة + حواف مدوّرة + إطار خفيف)
	local card = Instance.new("Frame")
	card.Name = "Card"; card.Size = UDim2.fromScale(1, 1)
	card.BackgroundColor3 = Color3.fromRGB(16, 18, 26); card.BackgroundTransparency = 0.03
	card.BorderSizePixel = 0; card.ZIndex = 1; card.Parent = bb
	Instance.new("UICorner", card).CornerRadius = UDim.new(0, 10)
	local cs = Instance.new("UIStroke")
	cs.Color = accent; cs.Thickness = 1.5; cs.Transparency = 0.25; cs.Parent = card

	-- شريط الاسم العلوي (بعرض البطاقة، مدوّر الأعلى فقط)
	local strip = Instance.new("Frame")
	strip.Name = "Strip"; strip.Size = UDim2.new(1, 0, 0, 32)
	...
	stripBot.Size = UDim2.new(1, 0, 0, 12); stripBot.Position = UDim2.new(0, 0, 1, -12)
	...
	-- الوصف (أفتح، تحت الشريط)
	local subLbl = Instance.new("TextLabel")
	subLbl.Name = "Subtitle"; subLbl.BackgroundTransparency = 1
	subLbl.Position = UDim2.new(0, 10, 0, 36); subLbl.Size = UDim2.new(1, -20, 0, 26)
	subLbl.Font = Enum.Font.GothamMedium; subLbl.TextScaled = true; subLbl.Text = sub
	subLbl.TextColor3 = Color3.fromRGB(210, 215, 230); subLbl.TextTransparency = 0.05
	subLbl.ZIndex = 2; subLbl.Parent = card
	Instance.new("UITextSizeConstraint", subLbl).MaxTextSize = 14

	-- خط أنيق رفيع بلون الدور أسفل البطاقة (بديل أنظف للنقاط)
	local bar = Instance.new("Frame")
	bar.Name = "AccentBar"; bar.Size = UDim2.new(0.5, 0, 0, 2)
	bar.Position = UDim2.new(0.25, 0, 1, -7); bar.AnchorPoint = Vector2.new(0, 0)
	bar.BackgroundColor3 = accent; bar.BackgroundTransparency = 0.35
	bar.BorderSizePixel = 0; bar.ZIndex = 2; bar.Parent = card
