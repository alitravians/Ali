...
local statusPanel = styled(statusLayer, {
	...
	-- أعلى الوسط ومُصغّر — حتى لا يغطي الدردشة أو أزرار الجوال
...
local function updateStatus()
	...
	if cuffed then
		...
		statusBody.Text = "الفك يتم فقط بواسطة صاحب الكلبشة أو الإدارة."
	...
	elseif holding then
		...
		statusBody.Text = "أنت تمسك لاعباً حالياً — حرّره من هنا متى شئت."
...
RunService.Heartbeat:Connect(function()
	if state.cuffedByUserId and state.cuffedByUserId ~= 0 then
		...
		statusPanel.Position = UDim2.new(0.5, 0, 0, 118 + math.floor(pulse * 18))
	else
		statusPanel.Position = UDim2.new(0.5, 0, 0, 118)
	end
end)

----------------------------------------------------------------------
-- وضعية التقييد الواقعية: تثبيت ذراعي أي لاعب مكلبش خلف ظهره
-- (الأنيميشن الافتراضي يحرّك الأذرع عبر Motor6D.Transform فوق وضعية
--  السيرفر — هنا نصفّر الـTransform كل إطار فتبقى اليدان مقيّدتين
--  حتى أثناء المشي/السحب، مع انحناءة خفيفة للجذع والرأس)
----------------------------------------------------------------------
local ARM_MOTOR_NAMES = {
	"RightShoulder", "Right Shoulder", "LeftShoulder", "Left Shoulder",
	"RightElbow", "LeftElbow", "RightWrist", "LeftWrist",
}
local WAIST_LEAN = CFrame.Angles(math.rad(7), 0, 0)
local NECK_LEAN = CFrame.Angles(math.rad(9), 0, 0)

type PoseEntry = { motor: Motor6D, cf: CFrame }
local poseCache: { [Model]: { PoseEntry } } = {}

local function collectPoseMotors(character: Model): { PoseEntry }
	local list: { PoseEntry } = {}
	for _, name in ipairs(ARM_MOTOR_NAMES) do
		local m = character:FindFirstChild(name, true)
		if m and m:IsA("Motor6D") then
			list[#list + 1] = { motor = m, cf = CFrame.identity }
		end
	end
	local waist = character:FindFirstChild("Waist", true)
	if waist and waist:IsA("Motor6D") then
		list[#list + 1] = { motor = waist, cf = WAIST_LEAN }
	end
	local neck = character:FindFirstChild("Neck", true)
	if neck and neck:IsA("Motor6D") then
		list[#list + 1] = { motor = neck, cf = NECK_LEAN }
	end
	return list
end
...
RunService.Stepped:Connect(function()
	for _, plr in ipairs(Players:GetPlayers()) do
		...
		if char and char:GetAttribute("CuffPosed") then
			local entries = poseCache[char]
			if not entries then
				entries = collectPoseMotors(char)
				poseCache[char] = entries
			end
			for _, e in ipairs(entries) do
				if e.motor.Parent then
					e.motor.Transform = e.cf
				end
			end
		end
	end
	for char in pairs(poseCache) do
		if not char.Parent or not char:GetAttribute("CuffPosed") then
			poseCache[char] = nil
		end
	end
end)
