...
claimRemote.OnServerEvent:Connect(function(player: Player)
	...
	-- حماية من الاستلام المزدوج (فورية بدون yield)
	claimedCache[uid] = true

	-- منح المكافأة أولاً قبل الحفظ في DataStore
	...
	-- حفظ في DataStore بالخلفية (لا يؤخّر اللاعب)
	task.spawn(function()
		if not claimStore then return end
		for _ = 1, 3 do
			if pcall(function() claimStore:SetAsync("rb_" .. uid, true) end) then return end
			task.wait(1)
		end
	end)
end)
