...
local function buildPlane(startPos: Vector3, travelDir: Vector3)
	...
	-- أرضية المقصورة (مرئية — خشب/سجادة ملكية)
	...
	local floor = collidableBox("CabinFloor",
		...
		boxCF(0, 0, FLOOR_U), Color3.fromRGB(48, 36, 30))
	floor.Material = Enum.Material.WoodPlanks
	-- ممشى سجادة ذهبي في المنتصف (تجميلي فقط)
	local runner = neonBulb("CabinRunner", Vector3.new(floorWid * 0.34, 0.12, floorLen * 0.96), GOLD, model)
	runner.Material = Enum.Material.Fabric
	runner.Transparency = 0.15
	runner.CFrame = boxCF(0, 0, FLOOR_U) * CFrame.new(0, 0.55, 0)
	...
	-- مقاعد ملكية على الجانبين (تجميلية، غير صلبة لتبقى الحركة سلسة)
	for _, side in ipairs({ -1, 1 }) do
		for i = -1, 1 do
			local seat = solid("CabinSeat", Enum.PartType.Block,
				Vector3.new(L * 0.07, R * 0.5, R * 0.5), Color3.fromRGB(70, 22, 32))
			seat.Material = Enum.Material.Fabric
			seat.CFrame = boxCF(i * 0.32, side * 0.62, FLOOR_U + 0.28)
			local cushion = solid("SeatBack", Enum.PartType.Block,
				Vector3.new(L * 0.07, R * 0.6, R * 0.12), GOLD)
			cushion.Material = Enum.Material.Fabric
			cushion.CFrame = boxCF(i * 0.32, side * 0.82, FLOOR_U + 0.5)
		end
	end

	-- إضاءة داخلية دافئة للمقصورة
	for i = -1, 1 do
		local glow = neonBulb("CabinLight", Vector3.new(R * 0.5, 0.18, R * 0.5), WARM, model)
		glow.CFrame = boxCF(i * 0.45, 0, CEIL_U - 0.06)
		mk("PointLight", { Color = WARM, Brightness = 1.4, Range = 22, Parent = glow, Shadows = false })
	end
...
local function run()
	...
	-- حلقة المقصورة: الطائرة ثابتة، نحدّث إضاءة LED وننتظر القفز
