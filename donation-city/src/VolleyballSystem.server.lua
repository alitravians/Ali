...
-- إطلاق الكرة لتسقط عند نقطة هدف بعد زمن طيران tf (حساب مقذوف صحيح مع الجاذبية)
-- هذا يضمن أن الكرة تعبر الشبكة فعلاً وتسقط داخل ملعب الخصم — لا على جهة المُرسِل.
local GRAVITY = workspace.Gravity   -- 196.2 افتراضياً
local function launchTo(target, tf)
	local p = ball.Position
	local vx = (target.X - p.X) / tf
	local vz = (target.Z - p.Z) / tf
	local vy = (target.Y - p.Y) / tf + 0.5 * GRAVITY * tf
	ball.AssemblyLinearVelocity = Vector3.new(vx, vy, vz)
	ball.AssemblyAngularVelocity = Vector3.new(math.random(-5, 5), 0, math.random(-5, 5))
end

-- ردّ الكرة لجهة الخصم بقوس عالٍ يسهل اللحاق به (لمسة لاعب أو إرسال)
local function hitBall(fromTeam, isServe)
	local toTeam = other(fromTeam)
	-- نقطة هدف عشوائية داخل ملعب الخصم (تجنّب الشبكة والخط الخلفي): لاعب يتحرّك يلحقها،
	-- وكرة بعيدة تُسجّل نقطة — فاللعب حقيقي لا مجرّد ردّ لا نهائي.
	local zLo, zHi
	if toTeam == "A" then zLo, zHi = VCZ - HALF_L + 4, VCZ - 4 else zLo, zHi = VCZ + 4, VCZ + HALF_L - 4 end
	local targetZ = zLo + math.random() * (zHi - zLo)
	local targetX = VCX + (math.random() - 0.5) * 2 * (HALF_W - 3)
	local tf = isServe and 1.3 or 1.15           -- زمن طيران كافٍ (قوس عالٍ) للحاق بالكرة
	launchTo(Vector3.new(targetX, FLOOR_Y + 1, targetZ), tf)
end
...
local HIT_RADIUS2 = 13 * 13         -- نصف قطر اللحاق بالكرة (أفقياً) ليردّها اللاعب — كريم ليسهل اللعب
...
task.spawn(function()
	while true do
		if match.phase == "rally" and not match.scoredLock then
			...
			-- نافذة واسعة طوال هبوط الكرة (Y حتى 18 والكرة هابطة vy<18) ليكفي وقوف اللاعب قربها
			local vy = ball.AssemblyLinearVelocity.Y
			if pos.Y > LAND_Y and pos.Y < 18 and vy < 18 and (os.clock() - lastHitClock) > 0.4 then
...
Players.PlayerRemoving:Connect(function(player)
	...
	if match.phase == "idle" then
		...
		updateBoard()
	elseif match.phase == "over" then
		-- المباراة انتهت والمكافآت سُلّمت — فقط نزيله من القائمة بلا إعادة احتساب فوز
		removeFrom(match.teamA, player.UserId)
		removeFrom(match.teamB, player.UserId)
	else
