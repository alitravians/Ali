...
----------------------------------------------------------------------
-- 🎮 ركن الألعاب — جناح مفتوح أنيق يحتضن الطاولات الثلاث، مفروش ومؤثّث.
--   مفتوح الجوانب تماماً (لا جدران/باب/قيود) فيلعب فيه كل اللاعبين.
--   كل القطع روبلوكس أصلية ومثبّتة (Anchored) = صفر لاق.
----------------------------------------------------------------------
local function buildGamesHall(parent: Instance)
	local hall = Instance.new("Model")
	hall.Name = "GamesHall"
	hall.Parent = parent

	local GC    = Vector3.new(26, 0, 70)     -- مركز الجناح (وسط الطاولات x=0/26/52)
	local PW    = 84                           -- العرض (محور X)
	local PD    = 30                           -- العمق (محور Z)
	...
	local function deco(props)
		props.Parent = hall
		if props.CanCollide == nil then props.CanCollide = false end
		return part(props)
	end

	-- أرضية مرمرية مرتفعة قليلاً تحت الطاولات
	deco({
		Name = "GamesFloor", CanCollide = true,
		Size = Vector3.new(PW, 0.6, PD),
		CFrame = CFrame.new(GC.X, 0.4, GC.Z),
		Color = Color3.fromRGB(236, 228, 214), Material = Enum.Material.Marble,
	})
	-- إطار نيون وردي حول الأرضية
	for _, e in ipairs({
		...
		deco({
			...
			CFrame = CFrame.new(GC.X + e[1].X, 0.7, GC.Z + e[1].Z),
			Color = PINK, Material = Enum.Material.Neon,
		})
	end

	-- عمود: جذع رخامي كريمي + قاعدة وتاج ذهبيان
	local function column(x: number, z: number)
		deco({
			Name = "Column", Shape = Enum.PartType.Cylinder, CanCollide = true,
		...
		for _, cy in ipairs({ 0.9, PH + 0.1 }) do
			deco({
				Name = "ColumnTrim", Shape = Enum.PartType.Cylinder,
				Size = Vector3.new(0.8, 2.4, 2.4),
				CFrame = CFrame.new(x, cy, z) * CFrame.Angles(0, 0, math.rad(90)),
				Color = GOLD, Material = Enum.Material.Metal,
			})
		end
	end
	-- أعمدة على الضلعين الجانبيين فقط → الواجهة الأمامية مفتوحة تماماً
	...
	-- السقف المسطّح (أسفله كحلي) + إطار ذهبي علوي
	deco({
		Name = "Roof", CanCollide = true,
		Size = Vector3.new(PW + 4, 1.4, PD + 4),
		CFrame = CFrame.new(GC.X, PH + 1.4, GC.Z),
		Color = NAVY, Material = Enum.Material.Slate,
	})
	deco({
		Name = "RoofTrim",
		Size = Vector3.new(PW + 6, 0.5, PD + 6),
		CFrame = CFrame.new(GC.X, PH + 2.2, GC.Z),
		Color = GOLD, Material = Enum.Material.Metal,
	})
	...
	local sign = deco({
		...
		CFrame = CFrame.new(GC.X, PH - 2.5, FRONT_Z),
		Color = Color3.fromRGB(18, 22, 36), Material = Enum.Material.SmoothPlastic,
	})
	for _, e in ipairs({
		...
		deco({
			...
			CFrame = CFrame.new(GC.X + e[1].X, PH - 2.5 + e[1].Y, FRONT_Z),
			Color = GOLD_HI, Material = Enum.Material.Neon,
		})
	end
	...
	-- 🪑 التفريش والتأثيث: سجادة دائرية + فانوس ذهبي معلّق فوق كل طاولة
	for _, origin in ipairs(TABLES) do
		...
		deco({
			...
			Color = Color3.fromRGB(60, 70, 120), Material = Enum.Material.Fabric,
		})
		deco({
		...
		deco({
			...
			CFrame = CFrame.new(tx, PH - 1.2, GC.Z),
			Color = GOLD, Material = Enum.Material.Metal,
		})
		deco({
			Name = "Lantern", Shape = Enum.PartType.Ball,
			Size = Vector3.new(2.2, 2.2, 2.2),
			CFrame = CFrame.new(tx, PH - 3.2, GC.Z),
	...
	for _, x in ipairs({ GC.X - PW / 2 + 2, GC.X + PW / 2 - 2 }) do
		for _, dz in ipairs({ -PD / 2 + 2, 0, PD / 2 - 2 }) do
			local pz = GC.Z + dz
			deco({
				Name = "Planter", CanCollide = true, Size = Vector3.new(2.6, 1.8, 2.6),
				CFrame = CFrame.new(x, 1.6, pz),
				Color = CREAM, Material = Enum.Material.Marble,
			})
			deco({
				Name = "PlanterRim", Size = Vector3.new(2.9, 0.4, 2.9),
				CFrame = CFrame.new(x, 2.5, pz),
				Color = GOLD, Material = Enum.Material.Metal,
			})
			deco({
				Name = "Bush", Shape = Enum.PartType.Ball,
				Size = Vector3.new(2.8, 2.4, 2.8),
				CFrame = CFrame.new(x, 3.7, pz),
				Color = Color3.fromRGB(86, 170, 96), Material = Enum.Material.Grass,
			})
		end
	end
