...
do
	...
	local function makeModal(titleText: string, w: number, h: number)
		...
		fitCard(card, w, h)
		-- النقر خارج البطاقة يُغلق النافذة (مع إبقاء زر X)
		root.InputBegan:Connect(function(io)
			if io.UserInputType == Enum.UserInputType.MouseButton1 or io.UserInputType == Enum.UserInputType.Touch then
				local p = io.Position
				local cp, cs = card.AbsolutePosition, card.AbsoluteSize
				if p.X < cp.X or p.X > cp.X + cs.X or p.Y < cp.Y or p.Y > cp.Y + cs.Y then
					playSound(SOUNDS.Click, SOUND_VOLUME)
					closeBoothModal()
				end
			end
