...
-- يضمن ألا تتجاوز أي نافذة (بطاقة) حدود الشاشة على أصغر الهواتف.
-- يضيف UIScale خاصاً بالبطاقة يصغّرها بشكل متناسب فقط عند اللزوم (بلا قصّ ولا
-- تمرير ولا فقدان محتوى)، ويعيد الحساب عند تدوير/تغيّر الشاشة. baseW/baseH هما
-- المقاس البكسلي المقصود للبطاقة.
local MODAL_MARGIN = 16
local function fitCard(card: GuiObject, baseW: number, baseH: number)
	if baseW <= 0 or baseH <= 0 then return end
	local fitScale = Instance.new("UIScale")
	fitScale.Name = "FitScale"
	fitScale.Parent = card
	local function apply()
		local cam = workspace.CurrentCamera
		if not cam then return end
		local g = uiScale.Scale
		if g <= 0 then g = 1 end
		-- المساحة المتاحة في إحداثيات الواجهة (قبل المقياس العام)
		local availW = cam.ViewportSize.X / g - MODAL_MARGIN
		local availH = cam.ViewportSize.Y / g - MODAL_MARGIN
		local s = math.min(availW / baseW, availH / baseH, 1)
		fitScale.Scale = math.clamp(s, 0.2, 1)
	end
	apply()
	local conns = {}
	local cam = workspace.CurrentCamera
...
local function makeModal(cardSize: UDim2)
	...
	TweenService:Create(card, TweenInfo.new(0.28, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
		{ Size = cardSize }):Play()
	fitCard(card, cardSize.X.Offset, cardSize.Y.Offset)
...
showAdminPanel = function(data)
	...
	local function buildShow()
		...
		-- زر مرحلة موحّد الستايل — يبرز الوضع المفعّل بإطار + نص ذهبي (مدمج/متناسق للجوال)
		...
		-- شبكة ٢×٢ مدمجة للمراحل الثابتة (أزرار أصغر تتناسق مع الجوال)
		local r1 = rowFrame(38)
		phaseButton(r1, "morning", "☀️ صباح", UDim2.new(0.5, -4, 1, 0), UDim2.fromOffset(0, 0))
		phaseButton(r1, "noon",    "🌞 ظهر",  UDim2.new(0.5, -4, 1, 0), UDim2.new(0.5, 4, 0, 0))
		local r2 = rowFrame(38)
		phaseButton(r2, "sunset", "🌇 مغرب", UDim2.new(0.5, -4, 1, 0), UDim2.fromOffset(0, 0))
		phaseButton(r2, "night",  "🌙 ليل",  UDim2.new(0.5, -4, 1, 0), UDim2.new(0.5, 4, 0, 0))
		-- زر «تلقائي» عريض — نص قصير عشان ما يطفح على الجوال (التفاصيل بالشريحة تحت)
		local r3 = rowFrame(38)
		phaseButton(r3, "auto", "🔄 تلقائي", UDim2.fromScale(1, 1), UDim2.fromOffset(0, 0))
		-- شريحة الحالة: الوضع الحالي + الساعة الفعلية
		local sf = new("Frame", {
			BackgroundColor3 = CARD2, Size = UDim2.new(1, 0, 0, 30), LayoutOrder = nextOrder(), Parent = content,
		}, { new("UICorner", { CornerRadius = UDim.new(0, 10) }), new("UIStroke", { Color = PURPLE, Transparency = 0.4 }) })
		new("TextLabel", {
			BackgroundTransparency = 1, Font = Enum.Font.GothamBold, TextSize = 12, TextColor3 = GOLD,
			Text = (PH_LABELS[activePhase] or activePhase) .. "  ·  " .. fmtHour(dn.hour),
			Size = UDim2.new(1, -16, 1, 0), Position = UDim2.fromOffset(8, 0),
			TextXAlignment = Enum.TextXAlignment.Right, Parent = sf,
		})
		note("يُطبَّق فوراً على الجميع ويتزامن عبر السيرفرات. للأدمن فأعلى فقط.")
...
do
	...
	local function makeModal(titleText: string, w: number, h: number)
		...
		local root = new("TextButton", {
			Name = "BoothModal", Text = "", AutoButtonColor = false, Modal = true,
		...
		fitCard(card, w, h)
		-- النقر على الخلفية (خارج البطاقة) يُغلق النافذة، مع إبقاء زر X.
		-- خلفية كزر كامل الشاشة (مثل makeModal الرئيسية): النقر على المناطق غير
		-- التفاعلية داخل البطاقة يمرّ للخلفية، وهو مستقل تماماً عن مقياس fitCard.
