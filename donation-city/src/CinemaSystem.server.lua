...
local function buildHallAmbiance()
	...
	-- شريط نيون أحمر خافت عند *قاعدة* الجدارين (مستوى الأرضية) فقط — لا يعبر خط النظر.
	-- الإصلاح الجذري: كان سابقاً على ارتفاع ٣ ستد (مستوى الشاشة/العين)، فمن زاوية
	-- المقاعد يظهر كخط أحمر يعبر أمام الشاشة، وتوهّجه (Bloom) يطمس لوحة شباك التذاكر
	-- المجاورة فتبدو مزدحمة. أنزلناه للأرضية + خفّفناه + قصّرناه بعيداً عن جدار الشاشة
	-- فلا يقطع الرؤية أبداً، مع بقاء لمسة الأجواء الحمراء كإضاءة قاعدة ناعمة.
	-- الإضاءة الأساسية للقاعة من أضواء السقف (أسفل) لا من هذه الألواح.
	local stripLen = fs.Z - 24
	neon(Vector3.new(0.3, 0.18, stripLen), Vector3.new(-wallX, topY + 0.2, cz), NEON_RED, 0.22, 7)
	neon(Vector3.new(0.3, 0.18, stripLen), Vector3.new( wallX, topY + 0.2, cz), NEON_RED, 0.22, 7)
	...
	if hasSlides then
		...
		withTimeout(PRELOAD_TIMEOUT, function()
			...
			for _, id in ipairs(slides) do
				probe.Image = "rbxthumb://type=Asset&id=" .. tostring(id) .. "&w=768&h=432"
