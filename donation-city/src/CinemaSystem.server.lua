...
local function playMovie(presser)
	...
	-- قصوى موحّدة حتى لا يتجمّد العرض إذا كان الأصل غير متاح (بلا إذن /
	-- معلّق على المراجعة / سيرفر بطيء). نمرّر «ميزانية» المهلة الكلية فلا
	-- تتراكم المهل عند تحميل عدّة أصول بالتتابع.
	local PRELOAD_TIMEOUT = 6 -- ثوانٍ كحدّ أقصى لكامل مرحلة تجهيز الأصول
