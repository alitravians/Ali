...
----------------------------------------------------------------------
-- ضابط شرطة بحريني عند بداية المسار (قطع أصلية، ثابت، يستقبل اللاعبين)
-- بدلة كحلية حديثة + بريه عنّابي + شارة كاب ذهبية + نجوم رتبة + علم البحرين،
-- وجه بملامح (عيون/حواجب/أنف/فم/أذنان/شعر). مبني بالكامل من Part/Cylinder
-- (صفر Union/Mesh). كل القطع Anchored وبلا تصادم/لمس → صفر فيزياء وصفر
-- استهلاك. يقف جانب نقطة الانطلاق على منصّة البداية ويواجه اللاعب القادم.
-- مرجع الهندسة: /home/ubuntu/police/officer.py (المعتمد من المستخدم).
----------------------------------------------------------------------
do
	-- ألوان البدلة (مطابقة لقيم المرجع المعتمد، محوّلة 0..1 → 0..255)
	local oNAVY   = C3(26, 33, 69)     -- كحلي البدلة
	local oNAVY2  = C3(33, 43, 84)     -- كحلي أفتح (تفاصيل/كتافيات/جيوب)
	local oMAROON = C3(107, 20, 36)    -- عنّابي البريه
	local oBLACK  = C3(13, 13, 15)     -- بوت/أربطة/منخار
	local oSKIN   = C3(219, 168, 128)  -- بشرة
	local oGOLD   = C3(219, 176, 84)   -- أزرار/شارات/نجوم
	local oSILVER = C3(204, 209, 217)  -- لوحة الاسم/مشبك الحزام
	local oWHITE  = C3(237, 237, 242)  -- أبيض العلم/بياض العين
	local oRED    = C3(199, 26, 36)    -- أحمر العلم
	local oEYE    = C3(26, 26, 36)     -- بؤبؤ/خط الفم
	local oBROW   = C3(41, 26, 15)     -- حاجب
	local oHAIR   = C3(18, 13, 13)     -- شعر
	local oLIP    = C3(148, 102, 97)   -- شفة
	local oIRIS   = C3(66, 41, 23)     -- قزحية
	...
	-- نجمة/شارة ذهبية مسطّحة تواجه الأمام: قرص (+ شعاعان متقاطعان للنجوم البارزة)
	local function ostar(bx, by, bz, r, prominent)
		ocyl(bx, by, bz, r * 0.55, 0.06, oGOLD, Enum.Material.Metal, "Y")
		if prominent then
			obox(bx, by, bz, r * 2.0, 0.05, r * 0.34, oGOLD, Enum.Material.Metal)
			obox(bx, by, bz, r * 0.34, 0.05, r * 2.0, oGOLD, Enum.Material.Metal)
		end
	end

	-- الأرجل: بنطلون كحلي + بوت أسود + ثنية أمامية
	for _, sx in ipairs({ -1, 1 }) do
		obox(sx * 0.46, 0, 1.35, 0.78, 0.92, 2.3, oNAVY)
		obox(sx * 0.46, -0.04, 0.28, 0.84, 1.04, 0.56, oBLACK)
		obox(sx * 0.46, 0.34, 1.0, 0.8, 0.18, 0.06, oNAVY2)
	end

	-- الجذع (سترة كحلية) + باكِت الأزرار + جيوب + لوحة اسم + نجمة صدر + ياقة
	obox(0, 0, 3.5, 2.05, 1.06, 2.05, oNAVY)
	obox(0, -0.55, 3.5, 0.16, 0.04, 1.9, oNAVY2)
	for k = 0, 3 do
		ocyl(0, -0.58, 4.15 - k * 0.5, 0.07, 0.05, oGOLD, Enum.Material.Metal, "Y")
	end
	for _, sx in ipairs({ -1, 1 }) do
		obox(sx * 0.55, -0.55, 3.05, 0.62, 0.05, 0.5, oNAVY2)
		obox(sx * 0.55, -0.56, 3.32, 0.4, 0.05, 0.06, oGOLD, Enum.Material.Metal)
	end
	obox(0.55, -0.57, 3.78, 0.6, 0.04, 0.16, oSILVER, Enum.Material.Metal)
	ostar(-0.55, -0.58, 3.8, 0.2, true)
	for _, sx in ipairs({ -1, 1 }) do
		obox(sx * 0.4, -0.5, 4.42, 0.5, 0.16, 0.16, oNAVY2)
	end

	-- الأذرع: كُمّ + يد + أساور + كتافية بنجمتي رتبة
	for _, sx in ipairs({ -1, 1 }) do
		obox(sx * 1.5, 0, 3.55, 0.74, 0.9, 1.95, oNAVY)
		obox(sx * 1.5, 0, 2.45, 0.7, 0.86, 0.42, oSKIN)
		obox(sx * 1.5, 0, 3.05, 0.76, 0.92, 0.12, oBLACK)
		obox(sx * 1.5, 0, 4.5, 0.78, 0.66, 0.12, oNAVY2)
		ostar(sx * 1.5, -0.18, 4.56, 0.12, false)
		ostar(sx * 1.5, 0.18, 4.56, 0.12, false)
	end

	-- علم البحرين على الكُمّ الأيمن (يُقرأ من الأمام): أحمر + أبيض + ٥ أسنان
	obox(1.5, -0.45, 3.98, 0.46, 0.05, 0.30, oRED)
	obox(1.34, -0.46, 3.98, 0.16, 0.05, 0.30, oWHITE)
	for n = 0, 4 do
		obox(1.45, -0.47, 3.85 + n * 0.072, 0.06, 0.05, 0.06, oWHITE, nil, CFrame.Angles(0, 0, math.rad(-45)))
	end

	-- الحزام + المشبك
	obox(0, 0, 2.5, 2.1, 1.1, 0.26, oBLACK)
	obox(0, -0.56, 2.5, 0.32, 0.06, 0.22, oSILVER, Enum.Material.Metal)

	-- الرقبة
	obox(0, 0.02, 4.55, 0.64, 0.62, 0.5, oSKIN)

	-- الرأس (جمجمة + فكّ/ذقن + أذنان)
	local cranium = obox(0, 0.02, 5.48, 1.40, 1.34, 1.04, oSKIN)
	obox(0, -0.03, 4.96, 1.16, 1.24, 0.70, oSKIN)
	for _, sx in ipairs({ -1, 1 }) do
		obox(sx * 0.72, 0.04, 5.36, 0.12, 0.34, 0.46, oSKIN)
	end

	-- الشعر (قصّة قصيرة تبيّن تحت البريه) + سوالف
	obox(0, 0.07, 5.66, 1.46, 1.40, 0.62, oHAIR)
	obox(0, -0.58, 5.90, 1.12, 0.16, 0.13, oHAIR)
	for _, sx in ipairs({ -1, 1 }) do
		obox(sx * 0.71, -0.26, 5.18, 0.10, 0.34, 0.52, oHAIR)
	end

	-- العيون (بياض + قزحية + بؤبؤ + جفن + حاجب بزاوية جدّية)
	for _, sx in ipairs({ -1, 1 }) do
		obox(sx * 0.31, -0.62, 5.47, 0.30, 0.08, 0.18, oWHITE)
		ocyl(sx * 0.31, -0.67, 5.47, 0.085, 0.05, oIRIS, Enum.Material.SmoothPlastic, "Y")
		ocyl(sx * 0.31, -0.70, 5.47, 0.035, 0.05, oEYE, Enum.Material.SmoothPlastic, "Y")
		obox(sx * 0.31, -0.65, 5.585, 0.34, 0.12, 0.07, oSKIN)
		obox(sx * 0.32, -0.70, 5.63, 0.36, 0.05, 0.085, oBROW, nil, CFrame.Angles(0, 0, math.rad(sx * 9)))
	end

	-- الأنف (جسر + طرف + منخاران)
	obox(0, -0.78, 5.42, 0.18, 0.18, 0.44, oSKIN)
	obox(0, -0.85, 5.18, 0.26, 0.22, 0.20, oSKIN)
	for _, sx in ipairs({ -1, 1 }) do
		obox(sx * 0.11, -0.82, 5.09, 0.05, 0.10, 0.06, oBROW)
	end

	-- الفم (شفتان رفيعتان + خطّ محايد)
	obox(0, -0.65, 5.01, 0.32, 0.09, 0.045, oLIP)
	obox(0, -0.65, 4.945, 0.30, 0.09, 0.055, oLIP)
	obox(0, -0.69, 4.98, 0.30, 0.04, 0.015, oEYE)

	-- البريه العنّابي (مائل) + رباط جلدي + شارة كاب ذهبية
	obox(0.05, 0.04, 5.88, 1.30, 1.26, 0.18, oBLACK)
	local beretTilt = CFrame.Angles(0, 0, math.rad(15)) * CFrame.Angles(math.rad(9), 0, 0)
	local flopTilt  = CFrame.Angles(0, 0, math.rad(24)) * CFrame.Angles(math.rad(12), 0, 0)
	ocyl(0.16, 0.10, 6.02, 0.99, 0.30, oMAROON, Enum.Material.Fabric, "Z", beretTilt)
	ocyl(0.66, 0.12, 6.12, 0.60, 0.16, oMAROON, Enum.Material.Fabric, "Z", flopTilt)
	ocyl(-0.46, -0.66, 6.02, 0.15, 0.06, oGOLD, Enum.Material.Metal, "Y")
	ostar(-0.46, -0.71, 6.02, 0.10, true)
