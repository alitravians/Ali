#!/usr/bin/env python3
# CDATA-preserving (lxml) insertion of the cinema-server character model
# ("Staff Worker M1G21", user-supplied Creator Store asset 249674776) as a
# hidden TEMPLATE under ServerScriptService, named "CinemaServerModel".
#
# The model is fully CLEAN (audited: zero Script/LocalScript/ModuleScript, zero
# loadstring/getfenv/HttpGet/require-by-id, zero ProtectedString Source — it is a
# plain R6 rig: Torso/arms/legs/Head + Humanoid + CharacterMeshes + Shirt/Pants +
# a chef-hat accessory as Handle+SpecialMesh), so per the project integration
# rule we adopt it AS-IS — we do NOT strip anything and we do NOT add game logic
# inside it. The server behaviour (name tag + ProximityPrompt + serving popcorn &
# drinks + idle sway) lives in our own CinemaSystem.server.lua, which clones this
# template and positions it at the popcorn stand.
#
# Same machinery as inject_guide.py (template under ServerScriptService, never
# replicates/renders, cloned into place at runtime — single source of truth).
#
# Idempotent: re-running removes any previous "CinemaServerModel" first.
import sys, copy, re
from lxml import etree

MAIN = "DonationCity_FINAL.rbxlx"
SRC  = "cinema_server_new.rbxmx"
MODEL_NAME = "CinemaServerModel"
PREF = "SERVER_"   # unique referent prefix to avoid clashes with the main file
...
def reprefix(model):
    ...
    for ref in model.iter("Ref"):
        if ref.text and ref.text.strip():
            # remap internal refs; nullify any ref pointing outside the model.
...
def main():
    ...
    # Merge SharedStrings blobs (md5-dedup): MeshParts reference mesh/physics data
    # by md5 in <SharedStrings>; without merging they dangle. This R6 rig uses
    # classic CharacterMesh/SpecialMesh (asset-id meshes, no SharedString blobs),
    # but keep this consistent with inject_guide/cashier/ticketbooth.
