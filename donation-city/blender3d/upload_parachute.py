...
FBX = os.path.join(os.path.dirname(__file__), "parachute.fbx")

url = "https://apis.roblox.com/assets/v1/assets"
req = {"assetType": "Model", "displayName": "ShahadRoyalParachute",
       "description": "Royal Descent parachute (navy+gold)", "creationContext": {"creator": {"userId": CREATOR_ID}}}
with open(FBX, "rb") as f:
    files = {"request": (None, json.dumps(req), "application/json"),
             "fileContent": ("parachute.fbx", f, "model/fbx")}
...
open(os.path.join(os.path.dirname(__file__), "parachute_asset_id.txt"), "w").write(str(asset_id))
