...
FBX = os.path.join(os.path.dirname(__file__), "laptop.fbx")

url = "https://apis.roblox.com/assets/v1/assets"
req = {"assetType": "Model", "displayName": "ShahadDeskLaptop",
       "description": "Executive desk laptop with wired mouse (graphite+gold)", "creationContext": {"creator": {"userId": CREATOR_ID}}}
with open(FBX, "rb") as f:
    files = {"request": (None, json.dumps(req), "application/json"),
             "fileContent": ("laptop.fbx", f, "model/fbx")}
...
open(os.path.join(os.path.dirname(__file__), "laptop_asset_id.txt"), "w").write(str(asset_id))
