#!/usr/bin/env python3
# ──────────────────────────────────────────────────────────────────────────
# Injector: 3D cherry blossom trees (أشجار كرز وردية 3D + تمايل مع الريح).
#   1) Script      "CherryTrees"       -> ServerScriptService
#      (source = src/CherryTrees.server.lua) — يبني الأشجار ويزيل الخضراء.
#   2) LocalScript "CherryTreesClient" -> StarterPlayerScripts
#      (source = src/CherryTrees.client.lua) — تمايل التيجان مع الريح.
#
...
SCRIPTS = [
    ("Script", "CherryTrees", "CherryTreesServerRef",
     os.path.join(HERE, "src", "CherryTrees.server.lua"), "ServerScriptService"),
    ("LocalScript", "CherryTreesClient", "CherryTreesClientRef",
     os.path.join(HERE, "src", "CherryTrees.client.lua"), "StarterPlayerScripts"),
]
