      ...
      <div className="relative w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="glass rounded-2xl p-8 space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 bg-gradient-to-r from-cyan-600 to-teal-600 hover:from-indigo-500 hover:to-purple-500 rounded-xl font-semibold text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
          ...
          <p className="text-center text-gray-400 text-sm">
            ليس لديك حساب؟{' '}
            <Link href="/register" className="text-cyan-400 hover:text-indigo-300">إنشاء حساب جديد</Link>
          </p>
        </form>
      </div>
    </div>
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-gray-950"><span className="w-8 h-8 border-2 border-cyan-500/30 border-t-indigo-500 rounded-full animate-spin" /></div>}>
      <LoginForm />
    </Suspense>
  );
}
