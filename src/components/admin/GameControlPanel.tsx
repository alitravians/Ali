...
const GameControlPanel: React.FC<GameControlPanelProps> = ({
  ...
  useEffect(() => {
    setIsOpen(gameStatus.isOpen);
    setClosureReason(gameStatus.closureReason || '');
  }, [gameStatus]);
  ...
  const handleSaveChanges = async (saveIsOpen = isOpen, saveClosureReason = closureReason) => {
    ...
    try {
      console.log('Saving game status:', saveIsOpen ? 'Open' : 'Closed');
      ...
      if (success) {
        ...
        console.error('Failed to save game status');
      }
    } catch (error) {
